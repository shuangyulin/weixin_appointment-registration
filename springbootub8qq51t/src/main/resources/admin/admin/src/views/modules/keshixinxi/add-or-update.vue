	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="剩余号源" prop="shengyuhaoyuan" >
					<el-input v-model.number="ruleForm.shengyuhaoyuan" placeholder="剩余号源" clearable  :readonly="ro.shengyuhaoyuan"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="剩余号源" prop="shengyuhaoyuan" >
					<el-input v-model="ruleForm.shengyuhaoyuan" placeholder="剩余号源" readonly></el-input>
				</el-form-item>
			...
			<el-form-item v-else-if="ruleForm.yuyueguize" label="预约规则" prop="yuyueguize" >
				<span class="text">{{ruleForm.yuyueguize}}</span>
			</el-form-item>
			<el-form-item v-if="type!='info'"  label="医生介绍" prop="yishengjieshao" >
				<editor 
					style="min-width: 200px; max-width: 600px;"
					v-model="ruleForm.yishengjieshao" 
					class="editor"
					myQuillEditor="yishengjieshao"
					action="file/upload">
				</editor>
			</el-form-item>
			<el-form-item v-else-if="ruleForm.yishengjieshao" label="医生介绍" prop="yishengjieshao" >
		...
		data() {
			...
			return {
				...
				rules: {
					...
					shengyuhaoyuan: [
						{ required: true, message: '剩余号源不能为空', trigger: 'blur' },
						{ validator: validateIntNumber, trigger: 'blur' },
					],
					yishengzhanghao: [
					],
			...
			init(id,type) {
				...
				this.$http({
					...
					if (data && data.code === 0) {
						...
						if(((json.zhuanyetezhang!=''&&json.zhuanyetezhang) || json.zhuanyetezhang==0) && this.$storage.get("role")!="管理员"){
							this.ruleForm.zhuanyetezhang = json.zhuanyetezhang
							this.ro.zhuanyetezhang = true;
						}
					} else {
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.yishengjieshao = this.ruleForm.yishengjieshao.replace(reg,'../../../springbootub8qq51t/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
