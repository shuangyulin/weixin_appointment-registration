<template>
	<div class="main-content" :style='{"width":"calc(100% - 40px)","padding":"20px 30px","margin":"20px 0 0 auto","fontSize":"16px","borderRadius":"10px","background":"#fff"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 10px 20px"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"20px 0","flexWrap":"wrap","background":"#fff","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('keshixinxi','科室统计')" type="success" @click="chartDialog1()">
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('keshixinxi','挂号')" @click="yuyueguahaoCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								挂号
							</el-button>
		...
		<el-dialog
			...
			width="800">
			<div id="keshifenleiChart1" style="width:100%;height:600px;"></div>
	...
	export default {
		...
		methods: {
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var keshifenleiChart1 = echarts.init(document.getElementById("keshifenleiChart1"),'macarons');
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							keshifenleiChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								keshifenleiChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
