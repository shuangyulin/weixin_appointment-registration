<template>
	<div class="main-content" :style='{"width":"calc(100% - 40px)","padding":"20px 30px","margin":"20px 0 0 auto","fontSize":"16px","borderRadius":"10px","background":"#fff"}'>
		<!-- 列表页 -->
			...
			<el-form class="center-form-pv" :style='{"margin":"0 10px 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","flexWrap":"wrap","background":"#fff","display":"flex"}' >
					<div :style='{"alignItems":"center","margin":"0 10px 10px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">科室名称</label>
					...
					<div :style='{"alignItems":"center","margin":"0 10px 10px 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">专业特长</label>
						<el-input v-model="searchForm.zhuanyetezhang" placeholder="专业特长" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"20px 0","flexWrap":"wrap","background":"#fff","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('yuyueguahao','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
						<span class="icon iconfont icon-zhifudingjin" :style='{"margin":"0 2px","fontSize":"14px","color":"#107ED6","height":"34px"}'></span>
						批量支付
					</el-button>
					<el-button class="btn18" v-if="isAuth('yuyueguahao','就诊人数统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-qita17" :style='{"margin":"0 2px","fontSize":"14px","color":"#107ED6","height":"34px"}'></span>
						就诊人数统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('yuyueguahao','挂号收入统计')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-qita17" :style='{"margin":"0 2px","fontSize":"14px","color":"#107ED6","height":"34px"}'></span>
						挂号收入统计
					</el-button>
				</el-row>
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"16px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						...
						label="挂号号源">
						<template slot-scope="scope">
							{{scope.row.shengyuhaoyuan}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="guahaozhuangtai"
						label="挂号状态">
						<template slot-scope="scope">
							{{scope.row.guahaozhuangtai}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="guahaoshijian"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('yuyueguahao','就诊提醒')" @click="jiuzhentixingCrossAddOrUpdateHandler(scope.row,'cross','否','是','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								就诊提醒
							</el-button>
							<el-button class="btn8" v-if="isAuth('yuyueguahao','取消')" @click="quxiaoguahaoCrossAddOrUpdateHandler(scope.row,'cross','否','是','guahaozhuangtai','已取消','已取消,已预约'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								取消
							</el-button>
		...
		<jiuzhentixing-cross-add-or-update v-if="jiuzhentixingCrossAddOrUpdateFlag" :parent="this" ref="jiuzhentixingCrossaddOrUpdate"></jiuzhentixing-cross-add-or-update>
		<quxiaoguahao-cross-add-or-update v-if="quxiaoguahaoCrossAddOrUpdateFlag" :parent="this" ref="quxiaoguahaoCrossaddOrUpdate"></quxiaoguahao-cross-add-or-update>
		...
		<el-dialog
			...
			width="800">
			<div id="shengyuhaoyuanChart1" style="width:100%;height:600px;"></div>
		...
		<el-dialog
			...
			width="800">
			<div id="guahaofeiChart2" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
...
<script>
	...
	import jiuzhentixingCrossAddOrUpdate from "../jiuzhentixing/add-or-update";
	import quxiaoguahaoCrossAddOrUpdate from "../quxiaoguahao/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				jiuzhentixingCrossAddOrUpdateFlag: false,
				quxiaoguahaoCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			jiuzhentixingCrossAddOrUpdate,
			quxiaoguahaoCrossAddOrUpdate,
		},
		methods: {
			imgPreView(url){
				this.previewImg = url
			...
			jiuzhentixingCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.jiuzhentixingCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.jiuzhentixingCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.jiuzhentixingCrossaddOrUpdate.init(row.id,type);
				});
			},
			quxiaoguahaoCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				if(crossOptPay=='是'&&row.ispay!='已支付'){
					this.$message({
						message: "请支付完成再操作",
						type: "success",
				...
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.quxiaoguahaoCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','yuyueguahao');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.quxiaoguahaoCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.quxiaoguahaoCrossaddOrUpdate.init(row.id,type);
				});
			},
			payHandler(row){
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var shengyuhaoyuanChart1 = echarts.init(document.getElementById("shengyuhaoyuanChart1"),'macarons');
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							shengyuhaoyuanChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								shengyuhaoyuanChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			chartDialog2() {
				...
				this.$nextTick(()=>{
					...
					var guahaofeiChart2 = echarts.init(document.getElementById("guahaofeiChart2"),'macarons');
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							guahaofeiChart2.setOption(option);
							//根据窗口的大小变动图表
							window.onresize = function() {
								guahaofeiChart2.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			getDataList() {
				...
				let params = {
 				...
 				if(this.searchForm.zhuanyetezhang!='' && this.searchForm.zhuanyetezhang!=undefined){
					params['zhuanyetezhang'] = '%' + this.searchForm.zhuanyetezhang + '%'
				}
				if(this.searchForm.keshifenlei!='' && this.searchForm.keshifenlei!=undefined){
					params['keshifenlei'] = this.searchForm.keshifenlei
				}
				if(this.searchForm.yuyueshijian!='' && this.searchForm.yuyueshijian!=undefined){
					params['yuyueshijian'] = this.searchForm.yuyueshijian
				}
				if(this.searchForm.shengyuhaoyuan!='' && this.searchForm.shengyuhaoyuan!=undefined){
					params['shengyuhaoyuan'] = this.searchForm.shengyuhaoyuan
				}
				if(this.searchForm.guahaozhuangtai!='' && this.searchForm.guahaozhuangtai!=undefined){
					params['guahaozhuangtai'] = this.searchForm.guahaozhuangtai
				}
